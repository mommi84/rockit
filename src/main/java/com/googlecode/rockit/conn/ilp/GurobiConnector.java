...
public class GurobiConnector extends ILPConnector{
	...
	@Override
	public void addVariable(String varName, double objWeight, double lowerBound, double upperBound,
			boolean override, boolean isZVar) throws ILPException {
		this.addVariablePrivate(varName, objWeight, lowerBound, upperBound, override, isZVar);		
	}
	
	private GRBVar addVariablePrivate(String varName, double objWeight, double lowerBound, double upperBound,
			boolean override, boolean isZVar) throws ILPException {
		GRBVar var = null;
		try{
			if(isZVar){
				...
				if(existingV != null){
					if(!override){
						...
						if(existingV.get(DoubleAttr.Obj)!=objWeight) this.objectiveVariablesToUpdate.put(existingV, objWeight);
						if(existingV.get(DoubleAttr.LB)!=objWeight) this.lowerBoundOfVariablesToUpdate.put(existingV, lowerBound);
						if(existingV.get(DoubleAttr.UB)!=objWeight) this.upperBoundOfVariablesToUpdate.put(existingV, upperBound);
					}
				}else{
					if(upperBound>1){
...
*/
	...
	public HashMap<String, Integer> optimizeILP() throws ILPException{
		...
		try{
			...
			if(Parameters.DEBUG_OUTPUT) System.out.print("Start ILP solver.");
			if(Parameters.DEBUG_OUTPUT) System.out.println(new Date());
			...
			if(Parameters.DEBUG_OUTPUT) model.update();
			if(Parameters.DEBUG_OUTPUT) model.write("model.lp");
			model.update();
	        ...
	        if (optimstatus == GRB.Status.INFEASIBLE) {
	  	    ...
	  	    if (optimstatus == GRB.Status.OPTIMAL) {
	            ...
	            if(Parameters.DEBUG_OUTPUT) System.out.println("Optimal objective: " + objval);		
				...
				if(Parameters.DEBUG_OUTPUT) System.out.print("Stop ILP solver. ");
				if(Parameters.DEBUG_OUTPUT) System.out.println(new Date());
	  	    }
		 } catch (GRBException e) {
			 e.printStackTrace(); 
