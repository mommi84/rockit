...
public class ObjectiveEvaluator {
	 ...
	 * 
	 * @param formula Formula for which the objective is computed.
	 * @param sql the sql connector.
	 * @return
	...
	public double getObjective(FormulaAbstract formula, MySQLConnector sql) throws ParseException, SQLException, SolveException{
		...
		if(Parameters.DEBUG_OUTPUT) System.out.print(fh);
		double weight = 0;
		if(formula instanceof FormulaSoft){
			if(Parameters.DEBUG_OUTPUT) System.out.print("weight (s) ");
			...
			if(Parameters.DEBUG_OUTPUT) System.out.print("weight (h) ");
			weight = 0;
			return (countTotal-countNegatives);
			//weight = Parameters.EVALUATION_WEIGHT_FOR_HARD_FORMULA;
		}				
		if(Parameters.DEBUG_OUTPUT) System.out.print(weight);
		if(Parameters.DEBUG_OUTPUT) System.out.print(" - total possible results: ");
		if(Parameters.DEBUG_OUTPUT) System.out.print(countTotal);
		if(Parameters.DEBUG_OUTPUT) System.out.print(" - recived results: ");
		if(Parameters.DEBUG_OUTPUT) System.out.print(countTotal-countNegatives);
		if(Parameters.DEBUG_OUTPUT && sumSoftPositive!=0) System.out.print(" - recieved sum of double var: ");
		if(Parameters.DEBUG_OUTPUT && sumSoftPositive!=0) System.out.print(sumSoftPositive);
		if(Parameters.DEBUG_OUTPUT) System.out.print(" - additional objective: ");
		if(Parameters.DEBUG_OUTPUT) System.out.print((countTotal-countNegatives)*weight + sumSoftPositive);
	 ...
	 * @param groundValueFile path to the ground value file.
	 * @param resultFile 
	 * @return
	 * @throws ParseException
	 * @throws SQLException
