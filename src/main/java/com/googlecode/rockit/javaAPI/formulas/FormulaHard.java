...
public class FormulaHard extends FormulaAbstract {
	...
	public FormulaHard(String name, HashSet<VariableType> forVariables, ArrayList<IfExpression> ifExpressions,
			ArrayList<PredicateExpression> restrictions, boolean usesConjunctions) throws ParseException {
	...
	private void checkRestrictionExpression(PredicateExpression ex) throws ParseException{
		if(ex.getClass().equals(PredicateExpression.class)){
			...
			if(exPred.getPredicate().isObserved()){
				throw new ParseException("Error in the restriction part of the formular. The predicate " + exPred.getPredicate().getName() + " is set to observed. But only hidden predicates are allowed in the restriction part.");
			}
		}
	}
